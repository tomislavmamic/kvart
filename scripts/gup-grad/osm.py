...
Izlaz: .cache/gup-grad/osm.json (sirovi odgovor Overpassa); cestice.py iz
njega gradi poligone. Podaci © OpenStreetMap contributors, ODbL.
...
def main() -> None:
    upit = UPIT.format(okvir=",".join(str(x) for x in OKVIR))
    zahtjev = urllib.request.Request(
        POSLUZITELJ,
        data=urllib.parse.urlencode({"data": upit}).encode(),
    ...
    with open(PUT, "w") as f:
        json.dump(d, f)
    print("OSM:", len(d["elements"]), "elemenata, stanje", d.get("osm3s", {}).get("timestamp_osm_base"))
