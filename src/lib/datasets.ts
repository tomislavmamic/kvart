...
export const DATASET_SECTIONS: DatasetSection[] = [
  {
    ...
    items: [
      ...
      },
      {
        ...
        desc: "Skenirane službene topografske karte; HOK je klasična osnovna karta 1:5000. TK25 je na karti kao podloga jer jedina nosi imena mjesta — Turnjevac, Karepovac, Sv. Spas, Meterize — kojih nema ni na ortofotu ni u katastru.",
        endpoints: [
      ...
      {
        name: "DGU reljef — DMR iz LiDAR-a (WCS)",
        desc: "Digitalni model reljefa, 1 m, bare-earth: zgrade su uklonjene, pa se vidi sam teren. Iz njega su izrađeni sjenčani reljef (podloga i preklopnik), izohipse od 2 m, visina i nagib u dosjeu čestice, te sloj „Kuda voda teče niz teren”.",
        endpoints: [
          "https://geoportal.dgu.hr/services/dmr/wcs  (coverageId=dmr__DMR_BW — pravi Float32)",
          "https://geoportal.dgu.hr/services/dmr/wms  (DMR_BW, DMR_COLOR, Hillshade — samo slika)",
        ],
        format: "WCS 2.0.1 → GeoTIFF (Float32)",
        license: "Otvorena dozvola (uz navođenje DGU)",
        coverage: "potpuna",
        status: "ok",
        statusLabel: "provjereno 14. 8. 2026.",
        note: "Za visine treba WCS: WMS na istom servisu vraća samo hipsometrijsku sliku, unatoč imenu sloja. Sjenčanje se poslužuje iz naših pločica, jer anonimni WMS otiskuje žig „GEOPORTAL” preko sredine svake pločice — a ondje se reljef i gleda.",
      },
      {
  ...
  },
  {
    ...
    items: [
      ...
      {
        name: "Katastarske čestice (DGU INSPIRE WFS)",
        desc: "Međe, broj, katastarska općina, službena površina i dan zadnje promjene svake čestice, iz iste žive baze koju prikazuje OSS preglednik. Gradski GIS izvoz ima čestice iz svibnja 2024.; od tada je promijenjeno 22 % čestica Grada. Iz ove su usluge čestice na karti i u dosjeu; analize vlasništva ostaju na česticama iz 2024., na kojima su provjerene. Skripta drži svih 71 354 čestica Grada u lokalnom spremištu i osvježava samo ono što se u međuvremenu promijenilo.",
        endpoints: [
          "https://api.uredjenazemlja.hr/services/inspire/cp/wfs  (cp:CadastralParcel, cp:CadastralZoning)",
        ],
        format: "WFS 2.0 / GML 3.2, EPSG:3765, najviše 1000 objekata po upitu",
        license: "bez uvjeta za pristup i korištenje (katalog OSS-a)",
        coverage: "potpuna",
        status: "ok",
        statusLabel: "provjereno 24. 9. 2026.",
        note: "Osvježavanje: npm run katastar:osvjezi (scripts/katastar/osvjezi.py). Vlasništva, načina uporabe i zgrada u ovoj usluzi nema; puni DKP WFS (oss.uredjenazemlja.hr/OssWebServices/wfs) ih ima, ali traži token koji DGU izdaje zasebno.",
      },
      {
...
export const DEAD_ENDS: { name: string; reason: string }[] = [
  ...
  { name: "Skupno vlasništvo čestica (OSS)", reason: "namjerno zatvoreno; javno je samo pojedinačno pretraživanje čestica. Same čestice otvorene su kroz INSPIRE WFS." },
